...
## Exercises
# Univariate
tbl_nsibs <- tbl_uvregression(
	nlsy,
	y = nsibs,
	include = sex_cat,
	method = lm)

tbl_wkdy <- tbl_uvregression(
	nlsy,
	y = sleep_wkdy,
	include = c(sex_cat),
	method = lm)

tbl_wknd <- tbl_uvregression(
	nlsy,
	y = sleep_wknd,
	include = c(sex_cat),
	method = lm)

tbl_income <- tbl_uvregression(
	nlsy,
	y = income,
	include = c(sex_cat),
	method = lm)

tbl_merge(list(tbl_nsibs, tbl_wkdy, tbl_wknd, tbl_income),
							 tab_spanner = c("**Nsibs**", "**Sleep Weekday**", "**Sleep Weekend**", "**Income**"))

# Poisson
poisson_model = glm(nsibs ~ eyesight_cat + sex_cat + race_eth_cat,
...
# Risk Ratios
# Log-binomial and Poisson models
...
tbl_eyes_binomial <- tbl_regression(
	eyes_binomial,
	exponentiate = TRUE,
	)

tbl_eyes_poisson <- tbl_regression(
	eyes_poisson,
	exponentiate = TRUE,
